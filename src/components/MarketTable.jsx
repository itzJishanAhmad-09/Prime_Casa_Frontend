...
const MarketTable = ({ marketData }) => {
  return (
    <div className="section" id="market">
          ...
          <tbody>
            {marketData.map((r, idx) => (
              <tr key={idx}>
                ...
                <td>
                  <span style={{fontSize:'11px', padding:'3px 8px', borderRadius:'8px', background:'var(--bg1)', border:'1px solid var(--border)'}}>
                    {r.cat}
                  </span>
                </td>
              </tr>
      ...
      <p style={{fontSize:'11px', color:'var(--txt3)', textAlign:'center', marginTop:'16px'}}>
        Source: NoBroker, 99acres — June 2026. Rates are indicative.
      </p>
    </div>
  );
};
